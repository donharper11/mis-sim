...
function AddWizard({ team, onClose, onSaved, instanceId }) {
  ...
  function selectChoice(key) {
    ...
    setChoiceKey(key); setPlacement(next?.placements[0]?.key || ""); setConfig(next?.configs[0]?.key || "");
  }
  ...
  async function submit(event) {
    ...
    try {
      const response = await apiClient.patch(`/instances/${instanceId}/components`, {
        ...
        replace_categories: { application: [{ key: `buy_${choiceKey}_${placement}`, op: "buy_application", catalog: choiceKey, placement, config, primary_for: null, tco_categories: [] }] },
      });
  ...
  return (
    <section className="components-wizard" aria-labelledby="components-wizard-heading">
      ...
      <div className="components-steps" aria-label="Wizard steps">{["Choose", "Placement", "Configuration", "Review"].map((label, index) => <span className={step === index + 1 ? "components-step--active" : step > index + 1 ? "components-step--done" : ""} key={label}>{index + 1}. {label}</span>)}</div>
      <form onSubmit={submit}>
        ...
        {step === 4 && <div className="components-review"><h3>{choice?.label}</h3><p>{placementNames[placement] || placement} · {config} · {mode ? `$${mode.capex.toLocaleString()} capex + $${mode.opex.toLocaleString()} per round` : "Choose a placement"}</p><p className="components-muted">This adds one application command to the current round sheet.</p></div>}
        <div className="components-wizard-actions"><button type="button" className="components-secondary" disabled={step === 1} onClick={() => setStep((value) => value - 1)}>Back</button>{step < 4 ? <button type="button" className="components-primary" disabled={(step === 1 && !choice) || (step === 2 && !placement) || (step === 3 && !config)} onClick={() => setStep((value) => value + 1)}>Continue</button> : <button type="submit" className="components-primary" disabled={!canSave || saving}>{saving ? "Saving…" : "Add to plan"}</button>}</div>
...
export default function Components({ data, instanceId }) {
  ...
  return <div className="components-page">
    ...
    {team.projects.length > 0 && <section className="components-projects"><h2>In flight</h2>{team.projects.map((project) => <p key={project.id}>{project.label} · {project.status} · {project.remaining_lead} round{project.remaining_lead === 1 ? "" : "s"} remaining</p>)}</section>}
  </div>;
}
