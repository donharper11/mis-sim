...
export default function Debrief({ data, instanceId }) {
  ...
  const score = latest.score || {}; const scorecard = latest.scorecard || {}; const changes = latest.state_changes || {}; const financials = latest.financials || {}; const model = latest.financial_model || {}; const debt = latest.technical_debt || {}; const freshness = latest.data_freshness || changes.data_freshness || {};
  const rationaleSummary = (response) => {
    const review = response.rationale_review;
    if (!review || review.status === "not_scored") return "review not scored";
    if (review.status === "scored") return `reviewed ${Math.round((review.quality || 0) * 100)}%`;
