...
export default function Debrief({ data, instanceId }) {
  ...
  const score = latest.score || {}; const scorecard = latest.scorecard || {}; const changes = latest.state_changes || {}; const financials = latest.financials || {}; const debt = latest.technical_debt || {};
  return <div className="debrief-page">
    ...
    <section className="debrief-causal"><div className="components-panel-heading"><div><h2>What changed</h2><p className="components-muted">The round result records what arrived, what was adopted, and what fired.</p></div><span className="components-muted">Round {latest.round}</span></div><div className="debrief-grid"><article><h3>State changes</h3><p>Arrived: {changes.arrived?.length ? changes.arrived.join(", ") : "none"}</p><p>Retired: {changes.retired?.length ? changes.retired.join(", ") : "none"}</p><p>Rollout changes: {changes.changed_rollouts?.length || 0}</p></article><article><h3>Signals and events</h3><p>Events: {latest.events.length ? latest.events.map((event) => event.label || event.key).join(", ") : "none"}</p><p>Prevented: {latest.prevented_events.length || 0}</p></article><article><h3>Financial result</h3><p>Capital spend: {financials.capital_spend ?? "—"}</p><p>Run-rate: {financials.opex_runrate ?? "—"}</p><p>Technical debt: {debt.closing ?? "—"}</p></article></div></section>
