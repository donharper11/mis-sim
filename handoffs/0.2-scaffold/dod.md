...
| Ladder rung 2 — build/lint clean | DEVIATION | Frontend `npm ci && npm run lint && npm run build` exited 0. Typecheck N-A: JSX-only project, no TypeScript. Dev-only `brace-expansion → minimatch → eslint / eslint-plugin-react` chain deferred; runtime `react-router` advisory accepted as AR-001 (see `SECURITY.md`). Vite reported bundle-size warning. |
