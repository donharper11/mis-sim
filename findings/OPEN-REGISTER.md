...
| G2 | **`design/02 §A` data currency/freshness remains a distinct factor**. Capture/storage settings and scoring consumption are still open; round-to-round production now has explicit immutable result evidence | ⏸ **PARTIALLY PRODUCED — M4 follow-up.** `round_result.data_freshness` records current-round producer, receiver, and required-entity status. Capture/storage settings remain owned by Platform, and scoring consumption remains a future follow-up. Component EOL `currency` stays distinct. |
