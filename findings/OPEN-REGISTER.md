...
| M2-006 | Separate `SimulationService` transaction leaves a theoretical lease-check TOCTOU window | supervisor; future M1 service-token seam | **OPEN, bounded risk accepted for local/cohort scheduling — 2026-09-16.** Scheduler performs a pre-call claim check and token-fenced schedule write; a strict atomic operation token would require an out-of-scope M1 service change. Close before internet-facing multi-worker scheduling. Evidence: `findings/recovery-m2-scheduling-2026-09-16.md`. |
