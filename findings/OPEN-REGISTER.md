...
| NS-004 | Per-round estate mutation remains supplied by scripts (`seeds/riverside_full.py:240–243` calls `_seed_round_estate` each round); adoption/decay/resistance producers remain unfinished | `recovery/decision-evolution` | **OPEN; M1 gate.** Inventory `0f08674` independently reproduced: no state carry-forward, unvalidated training targets, late duplicate-advance refusal, incomplete rewind and caller-committable partial arrivals. Commands/evidence: `handoffs/recovery/decision-evolution/{inventory,review}.md`. **M1 underway; P0 independently audited and integrated.** Its complete legacy24 payloads remain unchanged; the master transition contract is in correction after RETURN (`MSR-001`–`MSR-004`). D1–D10 are being settled under `handoffs/recovery/decision-evolution/authority.md`, with fresh review before bounded builders. Exit remains one initial estate plus six decision-only sheets with atomicity, accounting, retry/failure/isolation proof. |
...
| MSR-001 | Master integration projection exposes unrelated source data/roles and can bypass receiver capacity/liveness | `m1_contract_author`; supervisor owns seam ruling | **IN CORRECTION; blocks P1–P6.** `dcb59f5` independently RETURNED. Explicit one-hop entity-access seam is authorized for specification/review; P0 remains separate. Current failure and closing checks: `handoffs/recovery/decision-evolution/master-review-1.md`. |
| MSR-002 | Master balanced/no-org R2 template adds recurring cost after actual R1 events make the operating forecast negative | `m1_contract_author` | **IN CORRECTION; blocks master acceptance.** Source-backed event/forecast probe fails; author must provide valid explicit decisions and retain the paired technology comparison. Rules cannot be weakened to pass a template. Closing checks in master-review-1.md. |
| MSR-003 | Legacy cheapest-fix quote counts prevention as repair and ignores new procurement/operating/horizon constraints | author; supervisor owns production quote semantics | **IN CORRECTION; blocks P4.** Bounded verified repair-candidate input seam approved for specification; unverified combinations must be explicitly unassessed. Legacy default/pins remain protected. Response-only false-actionability witness and closing checks in master-review-1.md. |
| MSR-004 | Shared effect/resource DTOs, canonical array ordering and completed view/null paths remain incomplete | `m1_contract_author` | **IN CORRECTION; blocks P1–P6.** Exact nested shapes, action joins, view examples and vocabulary homes required before builders. Closing reference/candidate checks in master-review-1.md. |
| P0BSR-001 | New serving-path exclusions lacked total container/null/edge-kind and legacy-combination semantics | `m1_contract_author` | **CLOSED at spec review.** Final `2bab351` freezes and independently tests valid/invalid exclusions, native-only exclusions and exact physical-edge kinds. Candidate implementation still requires a fresh audit. Evidence: `handoffs/recovery/decision-evolution/production-inputs/review.md`. |
| P0BSR-002 | New access_via evidence lacked an exact JSON home, ordering, empty and valid-grant filter contract | `m1_contract_author`; supervisor applies living text | **CLOSED at spec review.** Final `2bab351` names the per-entity home and complete row/filter/order contract; wrong-home, dangling/order and legacy-empty-key defects are independently detected. Evidence: `handoffs/recovery/decision-evolution/production-inputs/review.md`. |
