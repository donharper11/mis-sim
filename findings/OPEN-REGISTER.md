...
| NS-006 | Runtime audit at `9ed2b94`: shared `.env.example` POSTGRES keys trigger Settings validation before connecting, and the exception echoes fixture values | `recovery/runtime` | **BEING FIXED.** Narrow scope amendment `6a6c707`; fresh-import regression, declared-setting validation retained, explicit verifier URL wins, real PostgreSQL verification from a temporary directory with shared dotenv shape. Re-audit required. |
