...
| G2 | **`design/02 §A` data currency/freshness remains a distinct factor**. Capture/storage settings and scoring consumption were previously open; round-to-round production now has explicit immutable result evidence | ✅ **M4 producer + consumer implemented.** Runtime packs author capture-enabled/storage-round settings, Platform exposes them, `round_result.data_freshness` records current-round producer/receiver/entity status, and Technology caps data adequacy by coverage. Component EOL `currency` stays distinct. |
...
**Register reconciliation 2026-09-15 — `OS-D1` CLOSED.** M2 packet 2.5 adds
field-aware `E19` numeric-range findings, the authoritative validator-spec entry,
the `broken_E19` fixture, and matrix evidence that the violation names the source
field and does not emit `E00`. The exact candidate `67a034d` passes the full gate
and independent audit. Evidence: `findings/recovery-m2-registry-2026-09-15.md`.
...
|---|---|---|---|
| NS-001 | `app.round.db.make_engine` strips `+asyncpg` and needs undeclared psycopg2; driver construction raises before connecting | `recovery/runtime` | **CLOSED on the integration branch — 2026-09-14.** Corrected candidate `ae634456` accepted; supervisor independently installed declared dependencies and verified real PostgreSQL migrations, 16 tables before seed and six committed rounds. Closing checks: `tests/test_runtime_dependencies.py`, `make check`, and disposable verifier recipe in `docs/backend-development.md`. Evidence: `recovery-runtime-2026-09-14.md`. |
| NS-002 | Inventory exact match omits strategies.yaml's no-space marker; 38 sites vs reported 37. Report shows only final-round BSC | `recovery/calibration-report` | **CLOSED on the integration branch — 2026-09-14.** Author-independent ACCEPT at `f858b8a`; 46 regressions pass (41 fail at baseline), all 96 BSC cells and 25 diagnostics match persisted values, score digest and 19 pack files unchanged. Closing command: from `backend/`, `PYTHONPATH=. python -m pytest tests/test_calibration_reporting.py -q`. Evidence: `recovery-calibration-report-2026-09-14.md`. Historical 37-site notes above are superseded by the current 38-site scan. |
| NS-003 | BSC base declares 0–1 (`engine/rollup.py:90`) but runner adds pack deltas directly (`round/runner.py:376`); Balanced R1 Financial -26.033068 | `recovery/scorecard-contract` | **CLOSED on the integration branch — 2026-09-14.** Reviewed implementation `5f5b09e` / final candidate `ad5de38` independently ACCEPTED. N1–N8 pass; all 96 BSC values reconcile with independently captured bases and point totals; every other field across 24 results remains unchanged. Balanced R1 Financial 0.696932 and actual partial-status evidence verified on persisted results. Closing commands: `make check` (311 tests/all guards/44 fixtures), focused `tests/test_scorecard_contract.py`, and actual PostgreSQL verifier/read-back. Evidence: `recovery-scorecard-2026-09-14.md`. Full Financial remains M4; transactions remain M1. |
| NS-004 | Per-round estate mutation remains supplied by scripts (`seeds/riverside_full.py:240–243` calls `_seed_round_estate` each round); adoption/decay/resistance producers remain unfinished | `recovery/decision-evolution` | **CLOSED at independent audit — 2026-09-15.** P0–P6 are integrated on the recovery branch. The decision-only boundary now carries one initial estate through six typed sheets with atomic persistence, accounting, retry/refusal, isolation and deterministic 16-game/96-report evidence. Historical legacy24 payloads remain unchanged. Evidence: `findings/recovery-games-2026-09-15.md`. |
| NS-005 | README/BATTLECARD and Phase 2 assumptions lag shipped state; capture/auth gate ordering conflicts | supervisor; M2 handoff authors | **CLOSED at pre-dispatch reconciliation — 2026-09-15.** Independent reconciliation at `376d54c` inventories the 19 live runtime tables, stale hierarchy/auth/registry/scheduling assumptions, and the M2 gate. The identity and pack tuple rulings, corrected 2.x references, and bounded 2.1 allowlist are committed at `376d54c`/`ba6e0a4`; later packets remain blocked on their named amendments. Evidence: `handoffs/recovery/m2-spec-reconciliation.md`, `handoffs/recovery/m2-contract-amendment.md`. |
| NS-006 | Runtime audit at `9ed2b94`: shared `.env.example` POSTGRES keys trigger Settings validation before connecting, and the exception echoes fixture values | `recovery/runtime` | **CLOSED on the integration branch — 2026-09-14.** `ae634456` independently re-audited; both fresh-import regressions fail before correction and pass after, known-field validation retained, real PostgreSQL verification succeeds with shared dotenv and no fixture-value echo. Closing command: from `backend/`, `PYTHONPATH=. python -m pytest tests/test_runtime_dependencies.py -q`. Evidence: `recovery-runtime-2026-09-14.md`. |
| M2-001 | 2.1 hierarchy service reads initially accepted an unscoped team/enrollment ID | `m2_hierarchy_builder`; supervisor | **CLOSED at independent successor audit — 2026-09-15.** Successor `ad199f5` requires `section_id` or `instance_id` on both reads, and focused negatives prove mismatched/unscoped reads fail. The final seed successor `5d6b573` retains the correction; audit also passes the migrated cohort and full 662-test gate. Evidence: `findings/recovery-m2-hierarchy-2026-09-15.md`. |
| M2-002 | 2.2 runtime scope boundary initially lacked an enforceable complete-inventory contract | `m2_scoping_builder`; supervisor | **CLOSED at independent successor audit — 2026-09-15.** Amended dispatch `35a7f3c` binds all 19 runtime tables to `simulation_instance` with restrictive FKs and orphan preflight, routes round/simulation reads through `ScopedRepo`, and proves two-instance isolation plus populated-instance delete refusal. Final candidate `da339c5` passes the schema guard, migration reversibility, focused regressions, 663-test full gate, and all guards. Evidence: `findings/recovery-m2-scoping-2026-09-15.md`. |
| M2-003 | 2.5 lacked a platform registry for validated runtime-capable casepacks and immutable instance pins | `m2_registry_builder`; supervisor | **CLOSED at independent successor audit — 2026-09-15.** Final candidate `67a034d` adds the metadata-only registry, `RuntimePackV1` resolution, semantic digest verification, setup-only binding, copied isolation fixture, and E19 closure. It passes the registry/schema/matrix evidence and the 666-test full gate. Evidence: `findings/recovery-m2-registry-2026-09-15.md`. |
| M2-004 | 2.4 auth contract lacked enforceable context, route, secret, seed, and login-surface boundaries | `m2_auth_builder`; supervisor | **CLOSED at independent successor audit — 2026-09-15.** Final successor `d89d5b6` adds the amended JWT/context contract, guarded hierarchy routes, deterministic 16/2/1 seed, required secret provisioning, and the M2 login surface. AUTH-001 (three instructors and unusable documented ownership) was corrected before final PASS. The exact candidate passes the browser/API evidence and 675-test full gate. Evidence: `findings/recovery-m2-auth-2026-09-15.md`. |
| M2-005 | 2.3 scheduling lacked a production-boundary, instance-safe participant, fixed-time, and concurrency contract | `m2_scheduler_builder`; supervisor | **CLOSED at independent successor audit — 2026-09-16.** Final successor `d41edb3` persists grace, validates settings, fences lock/advance service calls with database lease tokens, and passes the reversible migration, cohort scheduling evidence, 7 focused tests, 682-test full gate, and all guards. Evidence: `findings/recovery-m2-scheduling-2026-09-16.md`. |
| M2-006 | Separate `SimulationService` transaction leaves a theoretical lease-check TOCTOU window | supervisor; future M1 service-token seam | **CLOSED at race-fix audit — 2026-09-16.** `d41edb3` added pre-call fencing; the follow-up race fix makes `SimulationService.lock`/`.advance` lock and verify the schedule claim inside the same mutation transaction. The new lock/advance regression passes and no check-to-mutation window remains. Evidence: `findings/recovery-m2-scheduling-2026-09-16.md`. |
| SC-SR-001 | Proposed same-model validation could preserve mutated invalid event outcomes | `recovery/scorecard-contract` author | **CLOSED at spec review**, `0716eb9`: explicit raw-mapping reconstruction; reviewer verified rejection of mutable-map/model-copy/model-construct corruption. Implementation independently verified at `ad5de38`; N3 rejects mutable-map/model-copy/model-construct corruption, and regression plants fail. NS-003 closes at integration. |
| SC-SR-002 | Proposed living 1.6 delta restricted outcome recording to events with bound nodes | `recovery/scorecard-contract` author | **CLOSED at spec review**, `0716eb9`: exact whole-loop replacement records every fired event, including verified node=None penalties; outage evidence alone is conditional. All-fired outcome preservation is verified in the 24-result reproduction (including node=None penalties); the exact whole-loop living delta lands with NS-003 closure. |
| SC-A-001 | Living schema changelogs named CONTRACTS without the §7-required links | supervisor | **CLOSED before integration — 2026-09-14.** Both relative scorecard-contract links added; fresh auditor verified targets and proved removal fails the closing check in `handoffs/recovery/scorecard-contract/audit.md`. No code change. |
| M1-P0-SR-001 | P0's first prebuild oracle compared expected values with themselves and passed a real min-to-sum defect | `m1_contract_author` | **CLOSED at spec review — 2026-09-14.** Exact spec `818c54a` computes against real consumers; independent min-to-sum injection now fails. Closing proof and seven detected plants: `handoffs/recovery/decision-evolution/engine-inputs/review.md`. **Implementation obligation also verified:** candidate `7dbb4c8` independently ACCEPTED; actual min-to-sum and other code mutations fail the shipped checks. Audit: `recovery-engine-inputs-2026-09-14.md`. |
| M1-P0-SR-002 | P0 living-document delta targeted a nonexistent design section | author; supervisor applies delta | **CLOSED at spec review — 2026-09-14.** `818c54a` explicitly names a new final subsection in the verified existing design/07 file. Exact target/declaration check passes in the P0 review. **Application obligation verified at `7dbb4c8`:** exact living text, v1 changelogs and resolved links independently checked and integrated with audited code. Audit: `recovery-engine-inputs-2026-09-14.md`. |
| MSR-001 | Master integration projection exposes unrelated source data/roles and can bypass receiver capacity/liveness | `m1_contract_author`; supervisor owns seam ruling | **CLOSED at independent master review — 2026-09-15.** Corrected one-hop EntityAccess contract and audited P0b implementation require the live receiver/integration route, preserve source ownership/roles and pass route/failure checks. Evidence: `handoffs/recovery/decision-evolution/master-review-2.md`, `findings/recovery-production-inputs-2026-09-15.md`. |
| MSR-002 | Master balanced/no-org R2 template adds recurring cost after actual R1 events make the operating forecast negative | `m1_contract_author` | **CLOSED at independent master review — 2026-09-15.** All four strategies and both paired plans pass the 96-round source forecast; removing the required ransomware prevention fails the R2 horizon check. Evidence: `master-review-2.md` and frozen `spec.md`/`verify.md` at `bee3977`. |
| MSR-003 | Legacy cheapest-fix quote counts prevention as repair and ignores new procurement/operating/horizon constraints | author; supervisor owns production quote semantics | **CLOSED at specification level; P0b implementation audited — 2026-09-15.** Verified credit-eligible candidates, unassessed exposure, frozen initial quote, later affordability and all-held-response status checks are explicit. Legacy None path remains byte-identical. Evidence: `master-review-2.md`, `findings/recovery-production-inputs-2026-09-15.md`. |
| MSR-004 | Shared effect/resource DTOs, canonical array ordering and completed view/null paths remain incomplete | `m1_contract_author` | **CLOSED at independent master review — 2026-09-15.** Exact DTOs, identity joins, collection orders, required/null views and deferred label homes are frozen and falsified by the successor probes. Evidence: `master-review-2.md`. |
| P0BSR-001 | New serving-path exclusions lacked total container/null/edge-kind and legacy-combination semantics | `m1_contract_author` | **CLOSED at independent spec review and audited build — 2026-09-15.** Final `2bab351` and integrated P0b candidate `1729a89` freeze and test valid/invalid exclusions, native-only exclusions and exact physical-edge kinds. Evidence: `production-inputs/review.md`, `findings/recovery-production-inputs-2026-09-15.md`. |
| P0BSR-002 | New access_via evidence lacked an exact JSON home, ordering, empty and valid-grant filter contract | `m1_contract_author`; supervisor applies living text | **CLOSED at independent spec review and audited build — 2026-09-15.** Final `2bab351` and integrated candidate `1729a89` name and test the per-entity home, complete row/filter/order contract, dangling/order behavior and legacy omission. Evidence: `production-inputs/review.md`, `findings/recovery-production-inputs-2026-09-15.md`. |
| P0B-BLD-001 | P0b builder read some historical master background files after the first implementation/preflight run | `m1_production_builder`; supervisor disposition | **OWNED procedural residue — 2026-09-15.** Builder disclosed the ordering deviation in `production-inputs/dod.md`; exact-base corrective replay and same-byte reapplication passed, but replay does not make the original ordering compliant. No product defect found. Evidence: `findings/recovery-production-inputs-2026-09-15.md`. |
| P1-SR-001 | P1 allowlist did not state plainly that the three absent simulation source paths were NEW files | `m1_contract_author`; supervisor | **CLOSED at independent review — 2026-09-15.** Successor `2ef1eac` explicitly marks all three source paths NEW, adds the exact-base absence/create preflight and two planted misclassification negatives; review-3 passed. No P1 implementation had started. Evidence: `handoffs/recovery/decision-evolution/master-review-3.md`. |
| P1-CT-001 | P1 typed command/runtime/checkpoint boundary returned through several bounded audit corrections for content, DTO, preference, numeric, join and inventory defects | `m1_p1_content_builder_v4`; supervisor | **CLOSED at independent audit — 2026-09-15.** Final successor `e4f4757` passed the exact six-file allowlist, all malformed-content/checkpoint probes, mutation restoration, `321` focused/P0/legacy tests, `618` full-gate tests, all guards and the zero-error Riverside validator. Evidence: `findings/recovery-content-types-2026-09-15.md`. |
| P2-EST-001 | P2 estate/resource/projection candidate returned for lifecycle, replacement, policy/timing, projection identity, acquisition and inventory defects | `m1_p1_content_builder_v4`; supervisor | **CLOSED at independent audit — 2026-09-15.** Successor `b4bfec9` passed all five corrected probes, mutation restoration, `331` combined pins, `628` full-gate tests, all guards, validator `0/0` and the exact five-file allowlist. Evidence: `findings/recovery-estate-2026-09-15.md`. |
| P3-ORG-001 | P3 organisation candidate returned for wage/support consequences, process reversion, duplicate governance commands, DTO carry-forward and repeated-support no-op | `m1_p1_content_builder_v4`; supervisor | **CLOSED at independent audit — 2026-09-15.** Amended successor `2c42f24` passed all corrected probes, mutation restoration, `338` combined pins, `635` full-gate tests, validator `0/0` and the documented four-file scope amendment. Evidence: `findings/recovery-organisation-2026-09-15.md`. |
| P4-CON-001 | P4 consequences candidate returned through bounded corrections for repair catalogue/metrics, funded prevention, debt/TCO, ledger stamping, duplicate keys, action joins, preview/result evidence and empty-round TCO | `m1_p1_content_builder_v4`; supervisor | **CLOSED at independent audit — 2026-09-15.** Final successor `f43ea7f8` passed the exact six-file allowlist, all direct contract probes, `346` combined P0–P4 pins, `643` full-gate tests, validator `0/0` and clean-tree checks. Evidence: `findings/recovery-consequences-2026-09-15.md`. |
| P5-PERSIST-001 | P5 persistence candidate returned for runtime verifier model-scope failure after the service/schema packet passed | `m1_p1_content_builder_v4`; supervisor | **CLOSED at independent audit — 2026-09-15.** Successor `58a75669` passed the cumulative seven-file scope, `536` simulation/legacy pins, strict 19-table migration/runtime checks, six committed PostgreSQL rounds, validator evidence and clean-tree checks. Evidence: `findings/recovery-persistence-2026-09-15.md`. |
| P6-GAMES-001 | P6 decision-game candidate returned for fixture/strategy/no-op/matrix gaps and predecessor serialization/affordability defects | `m1_p1_content_builder_v4`; supervisor | **CLOSED at independent audit — 2026-09-15.** Final candidate `3855188` passed the exact three-file scope, five focused tests, 16-game/96-report matrix, fresh-scope determinism, causal cross-effects, matching `PYTHONHASHSEED=0/2` digests, root `652`-test gate and all guards. Evidence: `findings/recovery-games-2026-09-15.md`. |
